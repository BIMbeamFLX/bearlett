...
// that mint's own "@" button on the Mint page.
const AddressDialog: Component<AddressDialogProps> = props => {
  ...
  const claim = async () => {
    ...
    // this branch's own index-0 note key signs the registration proof
    // (the kit's signAddressProof, bound to the mint's domain and the
    // username) - required by the draft even for a fresh claim
    ...
    try {
      ...
      // re-renders straight into the manage view below
    } catch (err) {
  ...
  // enabling auto-check is the one user gesture to hang a Notification
  // permission prompt off of - most browsers silently drop a request made
  // outside a direct gesture
  const setAutoCheck = (
