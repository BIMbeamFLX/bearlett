...
export type SendToPubkeyDialogProps = {
  ...
  // to share one issuing mint by Wallet.tsx's own selection rules, same
  // precondition Combine and Combine & split rely on
  bearers: Bearer[]
  onClose: () => void
}

// LUD-25 Part 2's own p1/p2 mechanism, pointed at a cp1 pasted in by hand
// instead of one this wallet derived from a registered cx1 branch the way
// the kit's payInternalTransfer does. Whoever holds the matching private
// key proves ownership later with a ck1 signature; this dialog never asks
// for or sees that key, only its public cp1 commitment.
//
// One or many input notes, naming the pasted cp1 as either the WHOLE output
// (amount left blank: a combine, or a plain send if there is one input) or
// the first of a two-output split (an amount typed in, with a fresh
// wallet-held secret coming back as change) - the same two mint primitives
// Combine and Combine & split already use.
const SendToPubkeyDialog: Component<SendToPubkeyDialogProps> = props => {
  ...
  const send = async () => {
    ...
    if (rawAmount !== '') {
      // whole sats only: a fractional sat would round to a sub-sat,
      // unspendable output
    ...
    try {
      ...
      if (amountMsat === sum) {
        ...
        // naming an outside pubkey instead of another note of our own
        ...
        try {
          ...
          if (outcome === 'live') throw err
          if (outcome === 'unknown') {
            // the recipient's output is never this wallet's to lose - only
            // the change secret is, so only that one rides out as a
            // stored-unverified note
            await addBearer({
          ...
          // unsigned for now (settleNote below recovers it)
        }
  ...
  const changePreviewMsat = createMemo(() => {
    const raw = amountSats().trim()
    if (raw === '') return null
    const sats = Math.trunc(Number(raw))
    if (!Number.isFinite(sats) || sats <= 0) return null
    const change = totalMsat() - sats * 1000
  ...
  return (
    <Dialog onClose={props.onClose}>
      ...
      />
      <Show when={changePreviewMsat()}>
        {change => (
          <p class="bearer-hint">
            You'll keep ~{msatToSats(change())} sats change back in this wallet.
          </p>
        )}
      </Show>
      <div class="btns">
