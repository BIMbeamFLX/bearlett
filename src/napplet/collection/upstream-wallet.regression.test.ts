 ...
 * only the card library itself can make pass. The fixes are in the library
 * since BIMbeamFLX/600BillionTimelockTCG@0d05c73: a dropped pending no longer
 * burns counters, a restore scans at least 2N+100 counters, and a trade stays
 * pending on 429 or any 5xx whatever the body says.
 */

describe('regression: card library behaviour fixed upstream', () => {
  it('a refused re-issue in a 300-card catalogue does not cut the restore short', async () => {
  ...
  it('a JSON error after the mint committed a trade keeps the move going', async () => {
