...
describe('regression: a refused re-issue during a move (S1)', () => {
  it('confirms a moved card only once the seed can restore it', async () => {
...
describe('regression: an interrupted move and "They were passed on" (B1)', () => {
  it('never offers the move token as a handover, and moves every card', async () => {
    const mint = new TestNutftMint()
  ...
  it('refuses a handover while the move is open, and strands no move token', async () => {
    const mint = new TestNutftMint()
