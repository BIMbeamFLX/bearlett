...
describe('regression: the move token in the device wallet', () => {
  it.fails(
    '(a) a lost answer: the token is not a handover, and "passed on" cannot drop it',
    async () => {
      ...
      mint.lost = 'trade'
      await expect(ui.session.migrate()).rejects.toBeInstanceOf(
        MigrationStopped
      )
  ...
  it.fails(
    '(b) no other card can be handed over while the move is open',
    async () => {
      const mint = new TestNutftMint()
  ...
  it.fails(
    '(c) another copy of the same card is never taken for the moved one',
    async () => {
      const mint = new TestNutftMint()
  ...
  it.fails(
    '(d) another account and a signed-out open leave a stopped move alone',
    async () => {
      const mint = new TestNutftMint()
