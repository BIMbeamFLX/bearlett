...
Das Portal auf GitHub Einundzwanzig. Jede Funktion ein eigenes Napplet, alles
...
Recherche vom 9. September 2026 (Quellen 13.4). Reviewer setzen Phantom, Rainbow,
...
Unternehmen ohne Bezug (Quellen 13.5).
...
## 6. Einundzwanzig-Portal

Gefunden: `HolgerHatGarKeineNode/einundzwanzig-portal`, MIT, heute aktualisiert,
Code hinter `portal.einundzwanzig.space` mit Meetups, Kursen und Bibliothek für die
deutschsprachige Bitcoin-Community. Stack: Laravel, Livewire, Flux und Flux Pro,
Tailwind 4, Vite, PostgreSQL, Redis; serverseitig `swentel/nostr-php` und
`ezadr/lnurl-php`. Flux Pro ist eine kostenpflichtige Lizenz. Das ältere Repository
unter `Einundzwanzig-Podcast/einundzwanzig-portal` ist archiviert. Verwandt:
`einundzwanzig-group`, ein Nostr-Community-Client (NIP-29, welshman clientseitig), der
als Paket in der Android-App "TWENTY ONE Companion" mit Zaps und Wallet läuft.

Deutung: Das Portal ist ein Laravel-Webangebot, kein Napplet-Shell. Drei Rollen sind
denkbar, und sie führen zu verschiedener Arbeit:

1. **Referenz** für die Tonlage eines community-nahen, aufgeräumten Bitcoin-Auftritts.
   Kostet nichts, betrifft nur _Pay_.
2. **Host**: Das Portal bettet eine Shell-Seite ein, die Napplets lädt. Braucht einen
   Shell-Vertrag (Kehto oder Hangar) im Portal und eine Resource-Policy.
3. **Verteilung**: Das Portal verlinkt installierbare Napplets. Braucht signierte
   Manifeste und einen Installationsweg.

Offen und vom Auftraggeber zu bestätigen: welche Rolle gemeint ist. Der Entwurf nimmt
bis dahin Rolle 1 an.

## 7. Composable: Grundregeln
...
## 8. Napplet-Katalog mit Archetypen und Intents
...
### 13.2 TCG600nap, Nappelin, napplet/naps
...
### 13.5 HyperFrames und Einundzwanzig
...
<https://github.com/HolgerHatGarKeineNode/einundzwanzig-portal>,
<https://github.com/HolgerHatGarKeineNode/einundzwanzig-group>, <https://portal.einundzwanzig.space>.
...
funktional. Kein Three.js; der Prototyp bleibt bei Stufe 1 und 2 aus 11.3.
...
## 15. Offene Punkte und Nachweise

1. Rolle des Einundzwanzig-Portals bestätigen: Referenz, Host oder Verteilung.
2. Anfrage-Antwort über zwei Intents in Kehto und Hangar nachweisen: Kaltstart des
   Kerns, Zustellung an den Standard-Handler, Verhalten bei zwei offenen Oberflächen.
3. Archetyp-Vorschläge `vault`, `collection`, `card`, `reveal`, `trade`, `overview`,
...
4. Ein gebautes _Öffnen_-Napplet mit Three.js messen: Bundle, Startzeit, Bilder pro
   Sekunde auf einem mittleren Android-Gerät.
5. BOLT12 auf den genutzten Mints prüfen: Version, Angebot mit und ohne Betrag,
   Beschreibung, `amount_paid` und `amount_issued` bei Mehrfachzahlung.
6. Resource-Policy: Blossom-Spiegel freigeben, 3-MB-Grenze, Verhalten bei
   Spiegelausfall und Hash-Abweichung.
7. Speicher: Kartenbild-Cache im Napplet, Quoten, Verhalten nach Neustart.
8. Fonts: Budget je eingebettetem Schnitt messen und entscheiden.
9. Übergabe: Schlüsselaustausch per QR oder Intent statt Einfügen des P2BK-Schlüssels.
10. `prefers-reduced-motion` und Tastaturbedienung in jedem Napplet nachweisen.
11. Prototyp: Sammlung, Karte, Öffnen, Weitergeben und Pay-Übersicht in einem
