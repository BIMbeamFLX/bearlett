# LNURLcash 0.19.3, and what replacing NutFT still needs

22 September 2026. The kit change is commit `3754657` on
`feat/lnurlcash-0.19.3`, branched from `main`. The branch is local.
...
note was read against lnurl-mint `66c77e8` and LUD-25 `265759f`. Kit 0.19.3
does not close the gaps listed there.

## What was built

`@lnurlcash/kit` moved from 0.18.2 to 0.19.3. That is the package dni
published on 22 September 2026 from
[lnurl-wallet v0.19.3](https://github.com/lnurlcash/lnurl-wallet/releases/tag/v0.19.3).
Bearlett re-exports it from `src/lnurlcash.ts`.
...
allowed, so one note becomes two and two notes become one. Kit 0.19.3 adds
